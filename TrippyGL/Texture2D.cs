...
namespace TrippyGL
{
    ...
    public class Texture2D : Texture, IMultisamplableTexture
    {
        ...
        public void SetData<T>(T[] data, int dataOffset, int rectX, int rectY, int rectWidth, int rectHeight, OpenTK.Graphics.OpenGL4.PixelFormat pixelFormat = 0) where T : struct
        {
            ...
            GraphicsDevice.BindTextureSetActive(this);
            GL.TexSubImage2D(TextureType, 0, rectX, rectY, rectWidth, rectHeight, pixelFormat == 0 ? PixelFormat : pixelFormat, PixelType, ref data[dataOffset]);
        }
        ...
        private protected void ValidateGetOperation<T>(T[] data, int dataOffset) where T : struct
        {
            ...
            if (data.Length - dataOffset < Width * Height)
